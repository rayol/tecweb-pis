...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
    ...
    private static class MapClass extends
        ...
        //List of sources
        private static String[] personalizedSources = null;
        ...
        public void map(IntWritable nid, PageRankNode node, Context context)
            ...
            if (node.getAdjacenyList().size() > 0) {
                ...
                for(int i = 0; i < personalizedSources.length; i++) {
                    float massToBeAdded = node.getPageRankAtIndex(i) - (float) StrictMath.log(list.size());
                    mass.add(massToBeAdded);
                }
        ...
        public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
            ...
            while (values.hasNext()) {
                ...
                if (n.getType().equals(PageRankNode.Type.Structure)) {
                    ...
                    for(int i = 0; i < personalizedSources.length; i++) {
                        mass.set(i, sumLogProbs(mass.get(i), n.getPageRankAtIndex(i)));
                    }
    ...
    private static class MapPageRankMassDistributionClass extends
            Mapper<IntWritable, PageRankNode, IntWritable, PageRankNode> {
        private String[] missingMass = null;
        ...
        public void setup(Context context) throws IOException {
            ...
            personalizedSources = conf.getStrings("sources");
        ...
        public void map(IntWritable nid, PageRankNode node, Context context)
            ...
            for(int i = 0; i < personalizedSources.length; i++ ) {
                if(Integer.parseInt(personalizedSources[i]) == nid.get()) {
                    Float missingMassFloat = 0.0f;
                    if(Float.parseFloat(missingMass[i]) > 0.0f) {
                        missingMassFloat = Float.parseFloat(missingMass[i]);
                    }

                    float jump = (float) (Math.log(ALPHA));
                    float link = (float) Math.log(1.0f - ALPHA)
                            + sumLogProbs(p.get(i), (float) (Math.log(missingMassFloat)));
    ...
    private static final String PERSONALIZED_SOURCES = "sources";
    ...
    public int run(String[] args) throws Exception {
        ...
        options.addOption(OptionBuilder.withArgName("sources").hasArg()
                .withDescription("personalized sources").create(PERSONALIZED_SOURCES));
        ...
        String personalizedSources = cmdline.getOptionValue(PERSONALIZED_SOURCES);
        ...
        // Iterate PageRank.
        for (int i = s; i < e; i++) {
            iteratePageRank(i, i + 1, basePath, n, personalizedSources);
        }
    ...
    private void iteratePageRank(int i, int j, String basePath, int numNodes,
        ...
        ArrayListOfFloatsWritable mass = phase1(i, j, basePath, numNodes, source);
        ArrayListOfFloatsWritable missing = new ArrayListOfFloatsWritable();

        //Split the sources
        String[] personalizedSources = source.split(",");

        for(int q = 0; q < personalizedSources.length; q++) {
            missing.add(1.0f - (float) StrictMath.exp(mass.get(q)));
        }
    ...
    private ArrayListOfFloatsWritable phase1(int i, int j, String basePath, int numNodes, String source) throws Exception {
        ...
        for(int o = 0; o < personalizedSources.length; o++) {
            mass.add(Float.NEGATIVE_INFINITY);
        }
        ...
        for (FileStatus f : fs.listStatus(new Path(outm))) {
            ...
            for(int m = 0; m < personalizedSources.length; m++) {
                mass.set(m, sumLogProbs(mass.get(m), fin.readFloat()));
    ...
    private void phase2(int i, int j, ArrayListOfFloatsWritable missing, String basePath, int numNodes, String source) throws Exception {
        ...
        String missingMass = "" + missing.get(0);

        for(int w = 1; w < missing.size(); w++) {
            missingMass += "," + missing.get(w);
        }
        ...
        job.getConfiguration().setInt("NodeCount", numNodes);
        job.getConfiguration().set("sources", source);
